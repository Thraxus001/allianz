...
export default function SectionHeading({ eyebrow, heading, body, light, align = "left", green }: Props) {
  const textAlign = align === "center" ? "text-center items-center" : "text-left items-start";
  return (
    <div className={`flex flex-col ${textAlign}`}>
      <span className={`eyebrow ${
        green ? "text-[var(--color-leaf-2)]" : light ? "text-[var(--color-leaf)]" : "text-[var(--color-current)]"
      }`}>
        {eyebrow}
      </span>
      <h2
        className={`mt-3 font-display text-4xl font-bold leading-[0.95] tracking-tight md:text-5xl ${
          green ? "text-[var(--color-leaf)]" : light ? "text-white" : "text-[var(--color-deepwater)]"
        }`}
      >
        {heading}
      </h2>
